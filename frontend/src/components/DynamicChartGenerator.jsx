...
const DynamicChartGenerator = ({ 
  ...
  const colorPalette = {
    ...
    info: '#06B6D4',         // Cyan
    danger: '#EF4444',       // Red
    pie: ['#8B5CF6', '#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#06B6D4', '#EC4899', '#8B5CF6']
  };
  ...
  const filteredData = useMemo(() => {
    ...
    return tableData.filter((item) => {
      for (const [column, filterValue] of Object.entries(filters)) {
        ...
        } else if (filterValue && typeof filterValue === 'string' && filterValue.trim() !== '') {
          // For simple string filters
          const itemValue = item[column];
          const matches = String(itemValue).toLowerCase().includes(String(filterValue).toLowerCase());
  ...
  console.log('Filtered data count:', filteredData.length, 'of', tableData.length);
  ...
  const processChartData = () => {
    ...
    try {
      ...
      if (chartType === 'pie') {
        ...
        const topCategoriesCount = sortedEntries.reduce((sum, [, count]) => sum + count, 0);
        ...
        if (othersCount > 0) {
          processedData.push({
            name: `Others (${Object.keys(aggregatedData).length - 10} categories)`,
            value: othersCount,
            total: totalCount,
  ...
  const fetchSavedViews = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const chartViews = allViews.filter(view => 
          ...
          <AreaChart {...commonProps}>
            ...
            <defs>
              <linearGradient id="colorGradient" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor={colorPalette.accent} stopOpacity={0.3}/>
                <stop offset="95%" stopColor={colorPalette.accent} stopOpacity={0.1}/>
              </linearGradient>
            </defs>
          </AreaChart>
        </ResponsiveContainer>
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-6">
      ...
      <motion.div
        ...
        className="mb-8"
      >
        <h2 className="text-3xl font-bold text-gray-900 mb-2">Dynamic Chart Generator</h2>
                ...
                if (column === '_global') {
                  return (
                    <div key={column} className="flex flex-wrap items-center gap-2">
                      <span className="text-orange-700 font-medium">Global Search:</span>
        ...
        {savedViews.length > 0 && (
          ...
          >
            <h3 className="text-lg font-semibold text-gray-800 mb-3">Saved Chart Views</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {savedViews.map((view, index) => {
                let chartConfig = null;
                try {
                  chartConfig = view.chart_config ? JSON.parse(view.chart_config) : null;
                } catch (e) {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-medium text-gray-800 truncate">{view.name}</h4>
                    ...
                    {chartConfig && (
                      <div className="text-xs text-gray-600 space-y-1">
                        <div>Type: <span className="font-medium">{chartConfig.type}</span></div>
                        <div>X-Axis: <span className="font-medium">{chartConfig.xAxis}</span></div>
                        {chartConfig.yAxis && (
                          <div>Y-Axis: <span className="font-medium">{chartConfig.yAxis}</span></div>
                        )}
                      </div>
                    )}
                  </motion.div>
      ...
      >
        <h3 className="text-xl font-semibold text-gray-800 mb-4">Chart Configuration</h3>
            ...
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h3 className="text-xl font-semibold text-gray-800 mb-2">
                  {generatedChart.type.charAt(0).toUpperCase() + generatedChart.type.slice(1)} Chart
                </h3>
                <p className="text-gray-600 text-sm">
                  {generatedChart.type === 'pie' 
                    ? `Showing top ${Math.min(10, generatedChart.data?.length || 0)} categories${generatedChart.data?.length > 10 ? ' (others grouped)' : ''}`
                    : `${generatedChart.data?.length || 0} data points displayed`
                  }
                </p>
              </div>
              <motion.button
      ...
      {(!tableData || tableData.length === 0) && (
        ...
        >
          ...
          <p className="text-gray-500">Please provide tableData prop to generate charts</p>
        </motion.div>
      )}
